...
class ViewSolutions extends React.Component{
    ...
    componentDidMount()
    {
        ...
        axios
        .get("http://127.0.0.1:3000/api/v1/solution/all/"+this.state.queryid)
        .then(response =>{
            //alert("success")
            console.log(response.data)   
      const solutiondata=(response.data).map((obj)=><Solutioncard key={obj.solutionId} data={obj} c={this.props.c} queryId={this.state.queryid} click={this.componentforrerender} />)
    ...
    componentforrerender()
    {
        ...
        .then(response =>{
            //alert("success")
            console.log(response.data)   
      const solutiondata=(response.data).map((obj)=><Solutioncard key={obj.solutionId} data={obj} c={this.props.c} queryId={this.state.queryid}  />)
      console.log(solutiondata)
      this.setState({solutions:solutiondata})
            this.props.c({createquery:false,viewquery:true,loginpage:false}) 
          })
    ...
    {
        e.preventDefault()
        alert("entered form affter submitting method "+this.state.queryid)
        axios
        .post('http://127.0.0.1:3000/api/v1/solution/create/'+this.state.queryid,this.state)
        .then(response =>{
          alert("success")
    ...
    render(){
        return(
           ...
           <div>{this.state.solutions}</div>
            ...
            <br />
             <button className="btn btn-success" style={{marginLeft: "40%",marginBottom:"2%"}} onClick={this.formaftersubmitting}>submit</button>
            </form>
            </body>
            </html>
        )

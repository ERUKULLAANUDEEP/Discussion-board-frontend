...
class Solutioncard extends React.Component{
    ...
    gettingvotes(){
      axios
      .get("http://127.0.0.1:3000/api/v1/solution/"+this.props.queryId+'/'+this.props.data.solutionId)
      .then(response =>{
        alert("success")
    ...
    editingvotes(votes){
      ...
      axios
      .put("http://127.0.0.1:3000/api/v1/solution/"+this.props.queryId+'/'+this.props.data.solutionId,data)
      .then(response =>{
        alert("success")
       this.props.click()
      })
...
*/}
<div  style={{borderRadius:"20px", boxShadow:"10px 10px 10px  LightBlue",marginLeft:"10%",marginRight:"13%",marginBottom:"1%"}}>
  <div>
  <span  style={{display:""}}>
  <p style={{marginRight:"",whiteSpace:"pre"}}>{this.props.data.username}   <button style={{}} className="btn btn-success" style={{marginLeft:"75%",marginTop:""}}onClick={this.gettingvotes}>vote</button> {this.props.data.upvotes} votes</p>
